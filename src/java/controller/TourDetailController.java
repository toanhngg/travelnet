...
public class TourDetailController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Tour tour1 = tourDB.get(tour_id);
        HttpSession session = request.getSession();
        session.setAttribute("tr", tour1);
        session.setAttribute("tid", tour_id);
        TourDBContext tr = new TourDBContext();
        ArrayList<Tour> tour = tr.list();
        CommentDBContext com = new CommentDBContext();
        ArrayList<Comment> cm = com.listComment(tour_id);
